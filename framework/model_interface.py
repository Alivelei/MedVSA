...
from framework.metrics.mixup import image_mix_up, image_cutmix
...
class ModelInterfaceModule(pl.LightningModule):
    def __init__(self, model, args):
        ...
        self.model = model(load_pre_model=args.load_pre_model, freeze_model=args.freeze_model, ans_size=ans_word_size)

        if args.select_mix == "img_mixup":
            self.img_mix = image_mix_up
        elif args.select_mix == "img_cutmix":
            self.img_mix = image_cutmix
    ...
    def forward(self, img, qus, speech_array):
        return self.model(img, qus, speech_array)
    ...
    def training_step(self, batch, batch_idx):
        ...
        predict_ans = self(image, qus, speech_array)
        ...
        self.log("train_loss_step", ans_loss, prog_bar=True, on_epoch=False)
    ...
    def configure_optimizers(self):
        optimizer = torch.optim.AdamW(self.parameters(), lr=self.hparams.args.tch_learning_rate, weight_decay=self.hparams.args.tch_weights_decay)
